...
struct ClockedEntry: TimelineEntry {
    let date: Date
    ...
    static let placeholder = ClockedEntry(
...
struct ClockedProvider: TimelineProvider {
    func placeholder(in context: Context) -> ClockedEntry { .placeholder }

    func getSnapshot(in context: Context, completion: @escaping (ClockedEntry) -> Void) {
        Task { @MainActor in
            completion(Self.makeEntries(at: [Date.now]).first ?? .placeholder)
        }
    }

    func getTimeline(in context: Context, completion: @escaping (Timeline<ClockedEntry>) -> Void) {
        Task { @MainActor in
            let now = Date.now
...
struct ClockedSmallWidget: Widget {
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: "ClockedSmallWidget", provider: ClockedProvider()) { entry in
            ClockedSmallWidgetView(entry: entry)
        }
        .configurationDisplayName("Clocked")
...
struct ClockedSmallWidgetView: View {
    let entry: ClockedEntry
        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .firstTextBaseline) {
                Text("CLOCKED")
                    ...
                    .layoutPriority(-1)     // the state label wins the space
                    .lineLimit(1)
                Spacer(minLength: 4)
                Text(stateLabel)
