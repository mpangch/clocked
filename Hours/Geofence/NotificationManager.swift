...
final class NotificationManager: NSObject, UNUserNotificationCenterDelegate {
    ...
    func sendArrivalPrompt() {
        ...
        content.title = "Hours"
        content.body = "You arrived at Work — clock in?"
    ...
    private func scheduleAwayPrompt(fireIn: TimeInterval, thresholdSeconds: TimeInterval) {
        ...
        let content = UNMutableNotificationContent()
        content.title = "Hours"
        content.body = "You've been away from Work for \(Fmt.dur(thresholdSeconds)). Are you ready to clock out?"
    ...
    func scheduleBreakNudge() {
        ...
        let content = UNMutableNotificationContent()
        content.title = "Hours"
        content.body = "Break time? You usually step away for ~\(Fmt.dur(Double(TimeMath.round5(stats.typBreakDurMin)) * 60)) around \(Fmt.minToClock(typ))."
