...
- Day detail sheet: proportional timeline (green work / amber break / gaps), per-session **clock-in and clock-out steppers (15m)** for finished shifts, and a clock-in editor for the live session (forgot-to-clock-in backdating; clamp: in ≤ (first-segment-end ?? now) − 5m; the Track “In” chip opens it) (clamps: in ≤ first-segment-end − 5m; out ≥ last-segment-start + 5m), segment list, Delete session, day totals.
