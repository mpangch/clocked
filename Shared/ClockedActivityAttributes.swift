...
struct ClockedActivityAttributes: ActivityAttributes {
    struct ContentState: Codable, Hashable {
        /// true while the current segment is a paid break
        var isOnBreak: Bool
        /// original clock-in time (for "since 9:41 AM")
        var clockIn: Date
        /// For the working state: `now − net worked so far`, so
        /// Text(timerInterval: netAnchor...far) renders a live paid-hours timer
        /// (paid time includes breaks, so it keeps ticking on a break).
        var netAnchor: Date
